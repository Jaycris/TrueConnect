...
<div>
    ...
    <div class="panel mt-6">
        <div class="mb-5 flex items-center justify-between">
            <h5 class="text-lg font-semibold dark:text-white-light">Package Type</h5>
            <a href="{{ route('pack-type.create') }}" class="btn btn-primary">Add Package Type</a>
        </div>
        @if(session('success'))
        ...
        <div class="mb-5">
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                    @foreach($packType as $packTypes)
                        <tr>
                            <td><input type="checkbox" class="form-checkbox" /></td>
                            <td>{{ $packTypes->id }}</td>
                            <td>{{ $packTypes->pack_type_name }}</td>
                            <td>{{ $packTypes->created_at->format('M d, Y') }}</td>
                            <td class="text-center">
                                <ul class="flex items-center gap-2">
                                <li>
                                        <a href="{{ route('pack-type.view', $packTypes->id) }}" x-tooltip="View">
                                    ...
                                    <li>
                                        <a href="{{ route('pack-type.edit', $packTypes->id) }}" x-tooltip="Edit">
                                    ...
                                    <li>
                                    <form action="{{ route('pack-type.delete', $packTypes->id) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Are you sure you want to delete this package type?')" x-tooltip="Delete" class="bg-transparent border-none p-0 m-0">
